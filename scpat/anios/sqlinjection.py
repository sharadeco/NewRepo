...
def fetch_records(datatype):
    ...
    if datatype == 'DEMAND' : #fetching the Demand Data from the tables 
        ...
        sql = str( " select newid() as [id],       "
                    ...
                    +" [Update_timestamp],           "
                    +" [Delete_Ind]                  "
                    +" from dbo.Anios_SummaryDetails WHERE year([Date]) >= '2019'"
                )
...
#                    +" DATEPART(month, dbo.[Anios_ForecastData].[Date])		    <= datepart(month,@datevar_CET)) )   AND  "
        ...
        if table == "Calculated":                
            sql = str(sql_timezone
                    ...
                    +"dbo.[Anios_CalForecastData].[Delete_Indicator] = 'F' and  dbo.[Anios_CalForecastData].[Username] not like '%,%'   and [Date]  >=DATEADD(MONTH, DATEDIFF(MONTH, 0,@datevar_CET), 0) ")             
